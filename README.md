...
- `hud/ultra_minimal_apple_frame_HUD_builder_RV_v1.jsx` - ultra minimal Apple-style premium frame preset without a reference image, with a clean rounded frame, soft safe guides, pill status controls, center focus mark, and restrained lens/media readout.
- `hud/tech_data_frame_HUD_builder_RV_v1.jsx` - tech data frame preset without a reference image, with segmented panels, large numeric readouts, vector connector lines, data grid, matrix panel, and signal histogram.
- `hud/soft_futuristic_dashboard_HUD_builder_RV_v1.jsx` - soft futuristic dashboard preset without a reference image, with glass-style panels, radial core rings, ambient grid, smooth waveform, soft connectors, and balanced sci-fi telemetry.
- `hud/bodycam_frame_HUD_builder_RV_v1.jsx` - rough documentary bodycam frame preset without a reference image, with REC/timecode, unit and case metadata, GPS/storage/audio panels, evidence chain status, rough frame marks, and sensor noise.
- `hud/animated_histogram_shape_builder_RV_v1.jsx` - animated histogram component builder with manual comp size, one filled Shape Path, baked looping path keyframes, smooth 180-point Bezier curve, and style-only controls.
...
- `hud/drone_night_vision_HUD_builder_RV_v1.jsx` - drone night vision HUD preset without a reference image, with green/white night mode mix, IR/gain status, target lock, thermal signature, scan grain, mini map, and night telemetry.
