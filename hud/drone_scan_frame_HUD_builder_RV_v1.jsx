#target aftereffects
#targetengine "DroneScanFrameHUDBuilderRVV1"

/*
    drone_scan_frame_HUD_builder_RV v1
    Adobe After Effects 2022+

    Procedural drone scan frame HUD overlay.
...
(function DroneScanFrameHUDBuilderRVV1() {
    var COMP_W = 3840;
    var COMP_H = 2160;
    var COMP_DURATION = 10;
    var FPS = 30;
    var COMP_BASE = "DRONE_SCAN_FRAME_HUD_MAIN";
    ...
    function drawScanTopStatus(comp) {
        drawFilledTag(comp, "HUD_scan_mode", "SCAN", 88, 62, 130, 58, COLOR_GREEN, COLOR_BLACK, 34);
        addText(comp, "HUD_scan_status", "AREA SCAN ACTIVE", 255, 110, 38, COLOR_GREEN, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_scan_gps", "GPS 3D  /  SAT 18", 775, 110, 34, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_scan_target", "OBJECTS  04", 1700, 110, 38, COLOR_ACCENT, ParagraphJustification.CENTER_JUSTIFY, 10);
        addText(comp, "HUD_scan_bat", "BAT 78%", 3220, 110, 34, COLOR_GREEN, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_scan_rc", "RC STRONG", 3460, 110, 34, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 8);
    }

    function drawScanGrid(comp) {
        var layer = makeShapeLayer(comp, "HUD_scan_grid");
        var grid = addGroup(layer, "grid");
        for (var i = 0; i <= 12; i++) {
            var x = 620 + i * 220;
            addLineToContents(grid, x, 330, x, 1770);
        }
        for (var j = 0; j <= 8; j++) {
            var y = 330 + j * 180;
            addLineToContents(grid, 620, y, 3220, y);
        }
        addStroke(grid, COLOR_DIM, 2, 24);
    }

    function drawScanFrame(comp) {
        var layer = makeShapeLayer(comp, "HUD_scan_analysis_frame");
        var c = addGroup(layer, "scan frame");
        var x = 980;
        var y = 520;
        var w = 1880;
        var h = 1010;
        var l = 210;
        addLineToContents(c, x, y, x + l, y);
        addLineToContents(c, x, y, x, y + l);
        addLineToContents(c, x + w, y, x + w - l, y);
        addLineToContents(c, x + w, y, x + w, y + l);
        addLineToContents(c, x, y + h, x + l, y + h);
        addLineToContents(c, x, y + h, x, y + h - l);
        addLineToContents(c, x + w, y + h, x + w - l, y + h);
        addLineToContents(c, x + w, y + h, x + w, y + h - l);
        addStroke(c, COLOR_GREEN, 5, 100);

        var mid = addGroup(layer, "center scan cross");
        addLineToContents(mid, 1760, 1025, 2080, 1025);
        addLineToContents(mid, 1920, 865, 1920, 1185);
        addEllipseToContents(mid, 1870, 975, 100, 100);
        addStroke(mid, COLOR_WHITE, 3, 65);
        addText(comp, "HUD_scan_frame_label", "SCAN WINDOW", 1920, 480, 34, COLOR_GREEN, ParagraphJustification.CENTER_JUSTIFY, 10);
    }

    function drawScanLines(comp) {
        var layer = makeShapeLayer(comp, "HUD_scan_lines");
        var lines = addGroup(layer, "horizontal scan lines");
        for (var i = 0; i < 34; i++) {
            var y = 560 + i * 28;
            var alphaWidth = i % 5 === 0 ? 1320 : 980;
            addLineToContents(lines, 1920 - alphaWidth * 0.5, y, 1920 + alphaWidth * 0.5, y);
        }
        addStroke(lines, COLOR_ACCENT, 2, 34);

        var sweep = addGroup(layer, "bright sweep");
        addLineToContents(sweep, 980, 1010, 2860, 1010);
        addLineToContents(sweep, 980, 1028, 2860, 1028);
        addStroke(sweep, COLOR_GREEN, 4, 90);
    }

    function drawScanDetectedObjects(comp) {
        var layer = makeShapeLayer(comp, "HUD_scan_detected_objects");
        var c = addGroup(layer, "detections");
        addRectToContents(c, 1290, 790, 220, 150, 8);
        addRectToContents(c, 2280, 720, 260, 180, 8);
        addRectToContents(c, 1735, 1220, 190, 130, 8);
        addRectToContents(c, 2480, 1210, 170, 120, 8);
        addStroke(c, COLOR_YELLOW, 4, 100);

        addText(comp, "HUD_scan_obj_01", "OBJ 01", 1400, 780, 28, COLOR_YELLOW, ParagraphJustification.CENTER_JUSTIFY, 4);
        addText(comp, "HUD_scan_obj_02", "OBJ 02", 2410, 710, 28, COLOR_YELLOW, ParagraphJustification.CENTER_JUSTIFY, 4);
        addText(comp, "HUD_scan_obj_03", "OBJ 03", 1830, 1210, 28, COLOR_YELLOW, ParagraphJustification.CENTER_JUSTIFY, 4);
        addText(comp, "HUD_scan_obj_04", "OBJ 04", 2565, 1200, 28, COLOR_YELLOW, ParagraphJustification.CENTER_JUSTIFY, 4);
    }

    function drawScanSidePanels(comp) {
        drawDronePanel(comp, "HUD_scan_altitude", 82, 410, 420, 190, "ALTITUDE", "120.4", "m", COLOR_ACCENT);
        drawDronePanel(comp, "HUD_scan_speed", 82, 640, 420, 190, "SPEED", "8.2", "m/s", COLOR_WHITE);
        drawDronePanel(comp, "HUD_scan_range", 82, 870, 420, 190, "SCAN RANGE", "450", "m", COLOR_GREEN);
        drawDronePanel(comp, "HUD_scan_overlap", 82, 1100, 420, 190, "OVERLAP", "72", "%", COLOR_YELLOW);

        drawDronePanel(comp, "HUD_scan_heading", 3338, 410, 420, 190, "HEADING", "045", "deg", COLOR_GREEN);
        drawDronePanel(comp, "HUD_scan_signal", 3338, 640, 420, 190, "SIGNAL", "92", "%", COLOR_GREEN);
        drawDronePanel(comp, "HUD_scan_quality", 3338, 870, 420, 190, "QUALITY", "HIGH", "", COLOR_ACCENT);
        drawDronePanel(comp, "HUD_scan_cells", 3338, 1100, 420, 190, "CELLS", "128", "", COLOR_WHITE);
    }

    function drawScanProgress(comp) {
        var panel = makeShapeLayer(comp, "HUD_scan_progress");
        var c = addGroup(panel, "progress panel");
        addRectToContents(c, 1050, 1605, 1740, 118, 10);
        addFill(c, COLOR_BLACK, 40);
        addStroke(c, COLOR_WHITE, 3, 62);
        addText(comp, "HUD_scan_progress_label", "SCAN PROGRESS", 1100, 1676, 30, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_scan_progress_value", "64%", 2700, 1676, 42, COLOR_GREEN, ParagraphJustification.RIGHT_JUSTIFY, 6);

        var bar = makeShapeLayer(comp, "HUD_scan_progress_bar");
        var bg = addGroup(bar, "bar bg");
        addRectToContents(bg, 1480, 1650, 920, 24, 4);
        addFill(bg, COLOR_DIM, 45);
        var fill = addGroup(bar, "bar fill");
        addRectToContents(fill, 1480, 1650, 590, 24, 4);
        addFill(fill, COLOR_GREEN, 100);
    }

    function drawScanBottomStatus(comp) {
        addText(comp, "HUD_scan_bottom_gps", "GPS 3D LOCK", 620, 2035, 32, COLOR_GREEN, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_scan_bottom_mode", "MODE AREA SCAN", 1120, 2035, 32, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_scan_bottom_objs", "OBJECTS 04", 1700, 2035, 32, COLOR_YELLOW, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_scan_bottom_grid", "GRID 128", 2200, 2035, 32, COLOR_ACCENT, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_scan_bottom_rc", "RC STRONG", 2720, 2035, 32, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 8);
    }

    function drawDroneScanFrameHUD(comp) {
        drawScanTopStatus(comp);
        drawDroneFrame(comp);
        drawScanGrid(comp);
        drawScanFrame(comp);
        drawScanLines(comp);
        drawScanDetectedObjects(comp);
        drawScanSidePanels(comp);
        drawScanProgress(comp);
        drawDroneCompass(comp);
        drawScanBottomStatus(comp);
    }
    ...
    app.beginUndoGroup("Drone Scan Frame HUD Builder RV v1");
    try {
        ...
        alert("Drone Scan Frame HUD Builder problems:\n\n- " + err.toString() + (err.line ? " line " + err.line : ""));
    } finally {
        app.endUndoGroup();
    }
})();
