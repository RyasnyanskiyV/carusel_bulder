#target aftereffects
#targetengine "SoftFuturisticDashboardHUDBuilderRVV1"

/*
    soft_futuristic_dashboard_HUD_builder_RV v1
    Adobe After Effects 2022+

    Procedural soft futuristic dashboard overlay.
...
(function SoftFuturisticDashboardHUDBuilderRVV1() {
    var COMP_W = 3840;
    var COMP_H = 2160;
    var COMP_DURATION = 10;
    var FPS = 30;
    var COMP_BASE = "SOFT_FUTURISTIC_DASHBOARD_MAIN";
    ...
    function addArcToContents(contents, cx, cy, rx, ry, startDeg, endDeg, steps) {
        var points = [];
        var count = Math.max(4, steps || 24);
        for (var i = 0; i <= count; i++) {
            var t = startDeg + (endDeg - startDeg) * (i / count);
            var a = t * Math.PI / 180;
            points.push([cx + Math.cos(a) * rx, cy + Math.sin(a) * ry]);
        }
        return addPolylineToContents(contents, points, false);
    }

    function drawSoftGlassPanel(comp, name, x, y, w, h, title, value, subtitle, accentColor) {
        var layer = makeShapeLayer(comp, name + "_panel");
        var c = addGroup(layer, "glass panel");
        addRectToContents(c, x, y, w, h, 24);
        addFill(c, COLOR_BLACK, 26);
        addStroke(c, COLOR_WHITE, 1.8, 28);

        var glow = addGroup(layer, "soft accent");
        addLineToContents(glow, x + 28, y + 24, x + w - 28, y + 24);
        addStroke(glow, accentColor || COLOR_ACCENT, 2.8, 68);

        addText(comp, name + "_title", title, x + 34, y + 58, 24, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, name + "_value", value, x + 34, y + 132, 58, accentColor || COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 2);
        addText(comp, name + "_subtitle", subtitle, x + 34, y + h - 30, 22, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 6);
    }

    function drawSoftOuterShell(comp) {
        var layer = makeShapeLayer(comp, "HUD_soft_outer_shell");
        var shell = addGroup(layer, "soft shell");
        addRectToContents(shell, 190, 145, 3460, 1870, 42);
        addFill(shell, COLOR_BLACK, 14);
        addStroke(shell, COLOR_WHITE, 1.8, 36);

        var inner = addGroup(layer, "inner frame");
        addRectToContents(inner, 260, 230, 3320, 1700, 30);
        addStroke(inner, COLOR_DIM, 1.1, 20);

        var corners = addGroup(layer, "quiet corners");
        addLineToContents(corners, 360, 320, 620, 320);
        addLineToContents(corners, 360, 320, 360, 520);
        addLineToContents(corners, 3480, 320, 3220, 320);
        addLineToContents(corners, 3480, 320, 3480, 520);
        addLineToContents(corners, 360, 1840, 620, 1840);
        addLineToContents(corners, 360, 1840, 360, 1640);
        addLineToContents(corners, 3480, 1840, 3220, 1840);
        addLineToContents(corners, 3480, 1840, 3480, 1640);
        addStroke(corners, COLOR_WHITE, 2.6, 74);
    }

    function drawSoftAmbientGrid(comp) {
        var layer = makeShapeLayer(comp, "HUD_soft_ambient_grid");
        var grid = addGroup(layer, "ambient grid");
        for (var i = 0; i <= 8; i++) {
            var x = 960 + i * 240;
            addLineToContents(grid, x, 430, x, 1580);
        }
        for (var j = 0; j <= 5; j++) {
            var y = 500 + j * 200;
            addLineToContents(grid, 900, y, 2940, y);
        }
        addStroke(grid, COLOR_DIM, 1, 10);

        var diagonals = addGroup(layer, "ambient diagonals");
        addLineToContents(diagonals, 900, 1580, 1420, 430);
        addLineToContents(diagonals, 2940, 1580, 2420, 430);
        addStroke(diagonals, COLOR_ACCENT, 1.2, 18);
    }

    function drawSoftTopBar(comp) {
        drawSoftGlassPanel(comp, "HUD_soft_session", 330, 105, 440, 112, "SESSION", "AURORA", "ACTIVE INTERFACE", COLOR_ACCENT);
        drawSoftGlassPanel(comp, "HUD_soft_signal", 1350, 105, 440, 112, "SIGNAL", "98.6%", "LOW LATENCY", COLOR_GREEN);
        drawSoftGlassPanel(comp, "HUD_soft_status", 2050, 105, 440, 112, "SYSTEM", "STABLE", "SYNCED", COLOR_WHITE);
        drawSoftGlassPanel(comp, "HUD_soft_timer", 3070, 105, 440, 112, "TIME", "02:14", "REMAINING", COLOR_ACCENT);

        addText(comp, "HUD_soft_brand", "SOFT FUTURISTIC DASHBOARD", 1920, 282, 28, COLOR_DIM, ParagraphJustification.CENTER_JUSTIFY, 12);
    }

    function drawSoftRadialCore(comp) {
        var cx = 1920;
        var cy = 1045;
        var layer = makeShapeLayer(comp, "HUD_soft_radial_core");
        var rings = addGroup(layer, "radial rings");
        addEllipseToContents(rings, cx - 345, cy - 345, 690, 690);
        addEllipseToContents(rings, cx - 245, cy - 245, 490, 490);
        addEllipseToContents(rings, cx - 92, cy - 92, 184, 184);
        addStroke(rings, COLOR_DIM, 1.8, 26);

        var arcs = addGroup(layer, "active arcs");
        addArcToContents(arcs, cx, cy, 345, 345, -120, -20, 32);
        addArcToContents(arcs, cx, cy, 345, 345, 35, 128, 32);
        addArcToContents(arcs, cx, cy, 245, 245, 160, 258, 30);
        addArcToContents(arcs, cx, cy, 245, 245, -8, 82, 30);
        addStroke(arcs, COLOR_ACCENT, 5, 92);

        var greenArcs = addGroup(layer, "green arcs");
        addArcToContents(greenArcs, cx, cy, 292, 292, 220, 315, 28);
        addArcToContents(greenArcs, cx, cy, 292, 292, 12, 58, 20);
        addStroke(greenArcs, COLOR_GREEN, 4, 85);

        var ticks = addGroup(layer, "radial ticks");
        for (var i = 0; i < 48; i++) {
            var a = (i * 7.5 - 90) * Math.PI / 180;
            var r1 = i % 4 === 0 ? 378 : 392;
            var r2 = 408;
            addLineToContents(ticks, cx + Math.cos(a) * r1, cy + Math.sin(a) * r1, cx + Math.cos(a) * r2, cy + Math.sin(a) * r2);
        }
        addStroke(ticks, COLOR_WHITE, 1.4, 35);

        var cross = addGroup(layer, "center cross");
        addLineToContents(cross, cx - 78, cy, cx - 22, cy);
        addLineToContents(cross, cx + 22, cy, cx + 78, cy);
        addLineToContents(cross, cx, cy - 78, cx, cy - 22);
        addLineToContents(cross, cx, cy + 22, cx, cy + 78);
        addStroke(cross, COLOR_WHITE, 2.4, 64);

        addText(comp, "HUD_soft_core_value", "72.8", cx, cy + 14, 96, COLOR_WHITE, ParagraphJustification.CENTER_JUSTIFY, 2);
        addText(comp, "HUD_soft_core_label", "HARMONY INDEX", cx, cy + 90, 25, COLOR_DIM, ParagraphJustification.CENTER_JUSTIFY, 8);
    }

    function drawSoftSidePanels(comp) {
        drawSoftGlassPanel(comp, "HUD_soft_left_energy", 325, 455, 460, 170, "ENERGY", "84%", "BALANCED OUTPUT", COLOR_GREEN);
        drawSoftGlassPanel(comp, "HUD_soft_left_flow", 325, 685, 460, 170, "FLOW", "12.4", "GB / SEC", COLOR_ACCENT);
        drawSoftGlassPanel(comp, "HUD_soft_left_temp", 325, 915, 460, 170, "THERMAL", "38.2", "CELSIUS", COLOR_WHITE);
        drawSoftGlassPanel(comp, "HUD_soft_left_noise", 325, 1145, 460, 170, "NOISE", "0.08", "CLEAN SIGNAL", COLOR_DIM);

        drawSoftGlassPanel(comp, "HUD_soft_right_depth", 3055, 455, 460, 170, "DEPTH", "240", "LAYERS", COLOR_ACCENT);
        drawSoftGlassPanel(comp, "HUD_soft_right_focus", 3055, 685, 460, 170, "FOCUS", "LOCK", "SUBJECT STABLE", COLOR_GREEN);
        drawSoftGlassPanel(comp, "HUD_soft_right_gain", 3055, 915, 460, 170, "GAIN", "+3.2", "SOFT LIMIT", COLOR_WHITE);
        drawSoftGlassPanel(comp, "HUD_soft_right_link", 3055, 1145, 460, 170, "LINK", "ON", "MESH ACTIVE", COLOR_GREEN);
    }

    function drawSoftWaveform(comp) {
        var layer = makeShapeLayer(comp, "HUD_soft_waveform");
        var shell = addGroup(layer, "waveform shell");
        addRectToContents(shell, 1010, 1510, 1820, 225, 24);
        addFill(shell, COLOR_BLACK, 28);
        addStroke(shell, COLOR_WHITE, 1.8, 30);

        var base = addGroup(layer, "wave base");
        addLineToContents(base, 1090, 1640, 2750, 1640);
        for (var i = 0; i <= 20; i++) {
            var x = 1090 + i * 83;
            addLineToContents(base, x, 1628, x, 1652);
        }
        addStroke(base, COLOR_DIM, 1.4, 45);

        var wave = addGroup(layer, "soft wave");
        var points = [];
        for (var j = 0; j <= 72; j++) {
            var px = 1090 + j * (1660 / 72);
            var py = 1592 + Math.sin(j * 0.28) * 42 + Math.sin(j * 0.67) * 18;
            points.push([px, py]);
        }
        addPolylineToContents(wave, points, false);
        addStroke(wave, COLOR_ACCENT, 4, 90);

        var glow = addGroup(layer, "green response");
        var gpoints = [];
        for (var k = 0; k <= 44; k++) {
            var gx = 1320 + k * (1180 / 44);
            var gy = 1668 - Math.abs(Math.sin(k * 0.21) * 60) - Math.abs(Math.sin(k * 0.51) * 25);
            gpoints.push([gx, gy]);
        }
        addPolylineToContents(glow, gpoints, false);
        addStroke(glow, COLOR_GREEN, 3, 76);

        addText(comp, "HUD_soft_wave_title", "SOFT SIGNAL CURVE", 1090, 1555, 24, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_soft_wave_value", "MEAN 64%     PEAK 91%     DRIFT 0.2", 2750, 1555, 23, COLOR_WHITE, ParagraphJustification.RIGHT_JUSTIFY, 8);
    }

    function drawSoftConnectors(comp) {
        var layer = makeShapeLayer(comp, "HUD_soft_connectors");
        var c = addGroup(layer, "soft connectors");
        addPolylineToContents(c, [[785, 540], [1040, 540], [1575, 835]], false);
        addPolylineToContents(c, [[785, 770], [1100, 770], [1635, 1195]], false);
        addPolylineToContents(c, [[3055, 540], [2800, 540], [2265, 835]], false);
        addPolylineToContents(c, [[3055, 770], [2740, 770], [2205, 1195]], false);
        addLineToContents(c, 1920, 1450, 1920, 1510);
        addStroke(c, COLOR_DIM, 1.5, 32);

        var nodes = addGroup(layer, "soft nodes");
        var pts = [[1040, 540], [1100, 770], [2800, 540], [2740, 770], [1920, 1450]];
        for (var i = 0; i < pts.length; i++) {
            addEllipseToContents(nodes, pts[i][0] - 8, pts[i][1] - 8, 16, 16);
        }
        addFill(nodes, COLOR_ACCENT, 72);
    }

    function drawSoftFooter(comp) {
        var layer = makeShapeLayer(comp, "HUD_soft_footer");
        var c = addGroup(layer, "footer rails");
        addLineToContents(c, 510, 1900, 1510, 1900);
        addLineToContents(c, 2330, 1900, 3330, 1900);
        addStroke(c, COLOR_WHITE, 1.5, 26);

        addText(comp, "HUD_soft_footer_left", "PROFILE  AURORA / INTERFACE CLEAN", 510, 1960, 24, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_soft_footer_center", "SOFT FUTURISTIC DASHBOARD", 1920, 1960, 24, COLOR_ACCENT, ParagraphJustification.CENTER_JUSTIFY, 12);
        addText(comp, "HUD_soft_footer_right", "SYNC  98.6% / LINK STABLE", 3330, 1960, 24, COLOR_DIM, ParagraphJustification.RIGHT_JUSTIFY, 8);
    }

    function drawSoftFuturisticDashboard(comp) {
        drawSoftOuterShell(comp);
        drawSoftAmbientGrid(comp);
        drawSoftTopBar(comp);
        drawSoftRadialCore(comp);
        drawSoftConnectors(comp);
        drawSoftSidePanels(comp);
        drawSoftWaveform(comp);
        drawSoftFooter(comp);
    }

    if (!app.project) app.newProject();
    app.beginUndoGroup("Soft Futuristic Dashboard HUD Builder RV v1");
    try {
        ...
        drawSoftFuturisticDashboard(comp);
        try {
        ...
        alert("Soft Futuristic Dashboard HUD Builder problems:\n\n- " + err.toString() + (err.line ? " line " + err.line : ""));
    } finally {
        app.endUndoGroup();
    }
})();
