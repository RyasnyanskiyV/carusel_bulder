#target aftereffects
#targetengine "DroneAdvancedHUDBuilderRVV1"

/*
    drone_advanced_HUD_builder_RV v1
    Adobe After Effects 2022+

    Procedural advanced drone telemetry HUD overlay.
...
(function DroneAdvancedHUDBuilderRVV1() {
    var COMP_W = 3840;
    var COMP_H = 2160;
    var COMP_DURATION = 10;
    var FPS = 30;
    var COMP_BASE = "DRONE_ADVANCED_HUD_MAIN";
    ...
    function drawDroneVerticalScale(comp, name, x, y, h, title, markerText, markerColor, leftSide) {
        addText(comp, name + "_title", title, x, y - 28, 28, COLOR_DIM, leftSide ? ParagraphJustification.LEFT_JUSTIFY : ParagraphJustification.RIGHT_JUSTIFY, 6);
        var layer = makeShapeLayer(comp, name + "_scale");
        var c = addGroup(layer, "scale");
        addLineToContents(c, x, y, x, y + h);
        for (var i = 0; i <= 10; i++) {
            var ty = y + i * (h / 10);
            var len = i % 2 === 0 ? 72 : 42;
            if (leftSide) addLineToContents(c, x, ty, x + len, ty);
            else addLineToContents(c, x - len, ty, x, ty);
        }
        addStroke(c, COLOR_WHITE, 3, 85);

        var marker = makeShapeLayer(comp, name + "_marker");
        var m = addGroup(marker, "marker");
        var my = y + h * 0.42;
        if (leftSide) {
            addPolylineToContents(m, [[x + 84, my], [x + 130, my - 36], [x + 130, my + 36]], true);
        } else {
            addPolylineToContents(m, [[x - 84, my], [x - 130, my - 36], [x - 130, my + 36]], true);
        }
        addFill(m, markerColor || COLOR_ACCENT, 100);
        addText(comp, name + "_value", markerText, leftSide ? x + 155 : x - 155, my + 14, 34, markerColor || COLOR_ACCENT, leftSide ? ParagraphJustification.LEFT_JUSTIFY : ParagraphJustification.RIGHT_JUSTIFY, 4);
    }

    function drawDroneAdvancedScales(comp) {
        drawDroneVerticalScale(comp, "HUD_drone_adv_alt_scale", 620, 570, 720, "ALT (m)", "120.4", COLOR_ACCENT, true);
        drawDroneVerticalScale(comp, "HUD_drone_adv_dist_scale", 3220, 570, 720, "DST (m)", "860", COLOR_YELLOW, false);
        addText(comp, "HUD_drone_alt_160", "160", 520, 585, 26, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 0);
        addText(comp, "HUD_drone_alt_80", "80", 535, 945, 26, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 0);
        addText(comp, "HUD_drone_alt_0", "0", 555, 1295, 26, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 0);
        addText(comp, "HUD_drone_dst_1000", "1000", 3320, 585, 26, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 0);
        addText(comp, "HUD_drone_dst_500", "500", 3320, 945, 26, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 0);
        addText(comp, "HUD_drone_dst_0", "0", 3320, 1295, 26, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 0);
    }

    function drawDroneGPSHealth(comp) {
        var panel = makeShapeLayer(comp, "HUD_drone_adv_gps_health");
        var c = addGroup(panel, "panel");
        addRectToContents(c, 1220, 220, 1400, 96, 10);
        addFill(c, COLOR_BLACK, 35);
        addStroke(c, COLOR_WHITE, 3, 62);

        addText(comp, "HUD_drone_adv_gps_title", "GPS HEALTH", 1270, 282, 26, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 6);
        addText(comp, "HUD_drone_adv_sat", "18 SAT", 1560, 282, 34, COLOR_GREEN, ParagraphJustification.LEFT_JUSTIFY, 6);
        addText(comp, "HUD_drone_adv_hdop", "HDOP 0.7", 1845, 282, 34, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 6);
        addText(comp, "HUD_drone_adv_home", "HOME SET", 2180, 282, 34, COLOR_GREEN, ParagraphJustification.LEFT_JUSTIFY, 6);
    }

    function drawDroneMissionStrip(comp) {
        var panel = makeShapeLayer(comp, "HUD_drone_adv_mission_strip");
        var c = addGroup(panel, "mission strip");
        addRectToContents(c, 690, 1458, 2460, 122, 10);
        addFill(c, COLOR_BLACK, 38);
        addStroke(c, COLOR_WHITE, 3, 65);

        addText(comp, "HUD_drone_adv_mission", "MISSION  SURVEY_GRID_02", 750, 1530, 30, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_drone_adv_wp", "WP  07 / 18", 1270, 1530, 30, COLOR_ACCENT, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_drone_adv_eta", "ETA  00:04:26", 1620, 1530, 30, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_drone_adv_area", "AREA  1.62 ha", 2075, 1530, 30, COLOR_GREEN, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_drone_adv_wind", "WIND  3.4 m/s NE", 2545, 1530, 30, COLOR_YELLOW, ParagraphJustification.LEFT_JUSTIFY, 8);
    }

    function drawDronePayloadPanel(comp) {
        var panel = makeShapeLayer(comp, "HUD_drone_adv_payload");
        var c = addGroup(panel, "payload panel");
        addRectToContents(c, 2920, 1185, 640, 238, 10);
        addFill(c, COLOR_BLACK, 42);
        addStroke(c, COLOR_WHITE, 3, 68);

        addText(comp, "HUD_drone_payload_title", "CAMERA / PAYLOAD", 2965, 1240, 28, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 6);
        addText(comp, "HUD_drone_payload_res", "4K 30", 2965, 1308, 30, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 6);
        addText(comp, "HUD_drone_payload_zoom", "ZOOM  1.0x", 2965, 1368, 30, COLOR_ACCENT, ParagraphJustification.LEFT_JUSTIFY, 6);
        addText(comp, "HUD_drone_payload_gimbal", "GIMBAL  -12 deg", 3285, 1368, 30, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 6);
    }

    function drawDroneObstacleArc(comp) {
        var layer = makeShapeLayer(comp, "HUD_drone_adv_obstacle_arc");
        var c = addGroup(layer, "arc");
        var cx = 1920;
        var cy = 1080;
        for (var i = 0; i < 28; i++) {
            var a = (-155 + i * (310 / 27)) * Math.PI / 180;
            var r1 = 510;
            var r2 = i % 4 === 0 ? 565 : 540;
            addLineToContents(c, cx + Math.cos(a) * r1, cy + Math.sin(a) * r1, cx + Math.cos(a) * r2, cy + Math.sin(a) * r2);
        }
        addStroke(c, COLOR_DIM, 3, 60);

        var dots = addGroup(layer, "obstacle dots");
        addEllipseToContents(dots, 1415, 955, 30, 30);
        addEllipseToContents(dots, 2380, 960, 30, 30);
        addEllipseToContents(dots, 2170, 610, 26, 26);
        addFill(dots, COLOR_YELLOW, 100);
        addText(comp, "HUD_drone_obstacle_label", "OBSTACLE CLEAR", 1920, 515, 28, COLOR_GREEN, ParagraphJustification.CENTER_JUSTIFY, 8);
    }

    function drawDroneAdvancedHUD(comp) {
        drawDroneTopStatus(comp);
        drawDroneFrame(comp);
        drawDroneCenterReticle(comp);
        drawDroneTelemetry(comp);
        drawDroneAdvancedScales(comp);
        drawDroneGPSHealth(comp);
        drawDroneMissionStrip(comp);
        drawDronePayloadPanel(comp);
        drawDroneObstacleArc(comp);
    ...
    app.beginUndoGroup("Drone Advanced HUD Builder RV v1");
    try {
        ...
        alert("Drone Advanced HUD Builder problems:\n\n- " + err.toString() + (err.line ? " line " + err.line : ""));
    } finally {
        app.endUndoGroup();
    }
})();
