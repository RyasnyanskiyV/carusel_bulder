#target aftereffects
#targetengine "VlogCameraFrameHUDBuilderRVV1"

/*
    vlog_camera_frame_HUD_builder_RV v1
    Adobe After Effects 2022+

    Procedural simple vlog camera frame overlay.
...
(function VlogCameraFrameHUDBuilderRVV1() {
    var COMP_W = 3840;
    var COMP_H = 2160;
    var COMP_DURATION = 10;
    var FPS = 30;
    var COMP_BASE = "VLOG_CAMERA_FRAME_MAIN";
    ...
    function drawVlogCorners(comp) {
        var layer = makeShapeLayer(comp, "HUD_vlog_frame");
        var c = addGroup(layer, "soft corners");
        var x1 = 260;
        var x2 = 3580;
        var y1 = 220;
        var y2 = 1940;
        ...
        var guides = addGroup(layer, "light thirds");
        addLineToContents(guides, 1400, 350, 1400, 1810);
        addLineToContents(guides, 2440, 350, 2440, 1810);
        addLineToContents(guides, 470, 835, 3370, 835);
        addLineToContents(guides, 470, 1325, 3370, 1325);
        addStroke(guides, COLOR_DIM, 2, 22);
    }

    function drawVlogTop(comp) {
        var dot = makeShapeLayer(comp, "HUD_vlog_rec_dot");
        var d = addGroup(dot, "rec dot");
        addEllipseToContents(d, 292, 118, 46, 46);
        addFill(d, COLOR_RED, 100);
        addText(comp, "HUD_vlog_rec", "REC", 365, 160, 48, COLOR_RED, ParagraphJustification.LEFT_JUSTIFY, 18);
        addText(comp, "HUD_vlog_timer", "00:03:28", 580, 160, 46, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 12);

        drawFilledTag(comp, "HUD_vlog_mode", "VLOG", 1510, 104, 170, 58, COLOR_ACCENT, COLOR_BLACK, 34);
        addText(comp, "HUD_vlog_quality", "4K 30", 1725, 153, 40, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 10);
        addText(comp, "HUD_vlog_stab", "STAB ON", 1950, 153, 34, COLOR_GREEN, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_vlog_wifi", "Wi-Fi", 2350, 153, 34, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 8);
    }

    function drawVlogBattery(comp) {
        var layer = makeShapeLayer(comp, "HUD_vlog_battery");
        var c = addGroup(layer, "battery outline");
        addRectToContents(c, 3345, 110, 180, 58, 6);
        addRectToContents(c, 3525, 130, 14, 20, 2);
        addStroke(c, COLOR_WHITE, 4, 100);

        var fill = addGroup(layer, "battery fill");
        addRectToContents(fill, 3360, 124, 118, 30, 3);
        addFill(fill, COLOR_GREEN, 100);
        addText(comp, "HUD_vlog_battery_text", "78%", 3290, 158, 34, COLOR_GREEN, ParagraphJustification.RIGHT_JUSTIFY, 6);
    }

    function drawVlogFaceBox(comp) {
        var layer = makeShapeLayer(comp, "HUD_vlog_face_tracking");
        var c = addGroup(layer, "face box");
        addRectToContents(c, 1510, 620, 820, 760, 28);
        addStroke(c, COLOR_ACCENT, 5, 92);

        var corner = addGroup(layer, "small face corners");
        addLineToContents(corner, 1585, 705, 1705, 705);
        addLineToContents(corner, 1585, 705, 1585, 825);
        addLineToContents(corner, 2255, 705, 2135, 705);
        addLineToContents(corner, 2255, 705, 2255, 825);
        addLineToContents(corner, 1585, 1295, 1705, 1295);
        addLineToContents(corner, 1585, 1295, 1585, 1175);
        addLineToContents(corner, 2255, 1295, 2135, 1295);
        addLineToContents(corner, 2255, 1295, 2255, 1175);
        addStroke(corner, COLOR_WHITE, 3, 85);

        addText(comp, "HUD_vlog_face_label", "FACE TRACKING", 1920, 1448, 34, COLOR_ACCENT, ParagraphJustification.CENTER_JUSTIFY, 8);
    }

    function drawVlogMic(comp) {
        addText(comp, "HUD_vlog_mic_label", "MIC", 300, 1808, 34, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 8);
        var layer = makeShapeLayer(comp, "HUD_vlog_mic_meter");
        var green = addGroup(layer, "green mic bars");
        var startX = 420;
        for (var i = 0; i < 18; i++) {
            var x = startX + i * 23;
            addRectToContents(green, x, 1776, 14, 44, 2);
        }
        addFill(green, COLOR_GREEN, 100);

        var yellow = addGroup(layer, "yellow mic bars");
        for (var j = 18; j < 22; j++) {
            var yx = startX + j * 23;
            addRectToContents(yellow, yx, 1776, 14, 44, 2);
        }
        addFill(yellow, COLOR_YELLOW, 100);

        addText(comp, "HUD_vlog_mic_db", "-12 dB", 965, 1808, 30, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 4);
    }

    function drawVlogBottom(comp) {
        var pill = makeShapeLayer(comp, "HUD_vlog_bottom_pill");
        var p = addGroup(pill, "pill");
        addRectToContents(p, 1370, 1858, 1100, 86, 43);
        addFill(p, COLOR_BLACK, 42);
        addStroke(p, COLOR_WHITE, 3, 70);
        addText(comp, "HUD_vlog_lens", "24mm", 1515, 1917, 32, COLOR_WHITE, ParagraphJustification.CENTER_JUSTIFY, 6);
        addText(comp, "HUD_vlog_iso", "ISO AUTO", 1798, 1917, 32, COLOR_DIM, ParagraphJustification.CENTER_JUSTIFY, 6);
        addText(comp, "HUD_vlog_aperture", "F2.8", 2075, 1917, 32, COLOR_WHITE, ParagraphJustification.CENTER_JUSTIFY, 6);
        addText(comp, "HUD_vlog_storage", "128 GB", 2340, 1917, 32, COLOR_GREEN, ParagraphJustification.CENTER_JUSTIFY, 6);
    }

    function drawVlogFocusTouch(comp) {
        var layer = makeShapeLayer(comp, "HUD_vlog_touch_focus");
        var c = addGroup(layer, "touch mark");
        addLineToContents(c, 1920, 1010, 1920, 1064);
        addLineToContents(c, 1920, 1096, 1920, 1150);
        addLineToContents(c, 1850, 1080, 1904, 1080);
        addLineToContents(c, 1936, 1080, 1990, 1080);
        addEllipseToContents(c, 1899, 1059, 42, 42);
        addStroke(c, COLOR_WHITE, 3, 70);
    }

    function drawVlogCameraFrame(comp) {
        drawVlogCorners(comp);
        drawVlogTop(comp);
        drawVlogBattery(comp);
        drawVlogFaceBox(comp);
        drawVlogFocusTouch(comp);
        drawVlogMic(comp);
        drawVlogBottom(comp);
    }

    if (!app.project) app.newProject();
    app.beginUndoGroup("Vlog Camera Frame HUD Builder RV v1");
    try {
        ...
        alert("Vlog Camera Frame HUD Builder problems:\n\n- " + err.toString() + (err.line ? " line " + err.line : ""));
    } finally {
        app.endUndoGroup();
    }
})();
