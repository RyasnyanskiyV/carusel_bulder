#target aftereffects
#targetengine "DroneMapInterfaceHUDBuilderRVV1"

/*
    drone_map_interface_HUD_builder_RV v1
    Adobe After Effects 2022+

    Procedural drone map interface HUD overlay.
...
(function DroneMapInterfaceHUDBuilderRVV1() {
    var COMP_W = 3840;
    var COMP_H = 2160;
    var COMP_DURATION = 10;
    var FPS = 30;
    var COMP_BASE = "DRONE_MAP_INTERFACE_HUD_MAIN";
    ...
    function drawMapTopStatus(comp) {
        drawFilledTag(comp, "HUD_map_gps", "GPS", 88, 62, 100, 58, COLOR_GREEN, COLOR_BLACK, 34);
        addText(comp, "HUD_map_gps_status", "3D LOCK  /  18 SAT  /  HDOP 0.7", 220, 110, 36, COLOR_GREEN, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_map_route_name", "ROUTE  SURVEY_GRID_04", 1540, 110, 38, COLOR_ACCENT, ParagraphJustification.CENTER_JUSTIFY, 10);
        addText(comp, "HUD_map_battery", "BAT 78%", 3200, 110, 36, COLOR_GREEN, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_map_rc", "RC STRONG", 3450, 110, 36, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 8);
    }

    function drawMapPanel(comp) {
        var panel = makeShapeLayer(comp, "HUD_map_main_panel");
        var c = addGroup(panel, "map panel");
        addRectToContents(c, 650, 300, 2540, 1420, 14);
        addFill(c, COLOR_BLACK, 44);
        addStroke(c, COLOR_WHITE, 4, 82);

        var grid = addGroup(panel, "map grid");
        for (var i = 1; i < 7; i++) {
            var gx = 650 + i * (2540 / 7);
            addLineToContents(grid, gx, 300, gx, 1720);
        }
        for (var j = 1; j < 5; j++) {
            var gy = 300 + j * (1420 / 5);
            addLineToContents(grid, 650, gy, 3190, gy);
        }
        addStroke(grid, COLOR_DIM, 2, 30);

        var area = addGroup(panel, "survey area");
        addPolylineToContents(area, [[980, 1380], [1180, 760], [1880, 570], [2730, 780], [2840, 1340], [2100, 1510], [1360, 1480]], true);
        addStroke(area, COLOR_ACCENT, 4, 85);

        var route = addGroup(panel, "route path");
        addPolylineToContents(route, [[1080, 1290], [1280, 1120], [1500, 1180], [1740, 980], [2020, 1040], [2280, 840], [2560, 930], [2750, 760]], false);
        addStroke(route, COLOR_GREEN, 5, 100);

        var current = addGroup(panel, "current route");
        addPolylineToContents(current, [[1080, 1290], [1280, 1120], [1500, 1180], [1740, 980]], false);
        addStroke(current, COLOR_YELLOW, 6, 100);

        var dots = addGroup(panel, "waypoints");
        var pts = [[1080, 1290], [1280, 1120], [1500, 1180], [1740, 980], [2020, 1040], [2280, 840], [2560, 930], [2750, 760]];
        for (var k = 0; k < pts.length; k++) {
            addEllipseToContents(dots, pts[k][0] - 14, pts[k][1] - 14, 28, 28);
        }
        addFill(dots, COLOR_WHITE, 100);

        var drone = addGroup(panel, "drone marker");
        addPolylineToContents(drone, [[1740, 930], [1705, 1018], [1775, 1018]], true);
        addFill(drone, COLOR_YELLOW, 100);

        addText(comp, "HUD_map_panel_title", "MISSION MAP", 710, 360, 34, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 10);
        addText(comp, "HUD_map_scale", "500 m", 2920, 1665, 30, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 6);
    }

    function drawMapSideStats(comp) {
        drawDronePanel(comp, "HUD_map_wp", 82, 410, 420, 190, "WAYPOINT", "07/18", "", COLOR_ACCENT);
        drawDronePanel(comp, "HUD_map_next", 82, 640, 420, 190, "NEXT WP", "186", "m", COLOR_WHITE);
        drawDronePanel(comp, "HUD_map_eta", 82, 870, 420, 190, "ETA NEXT", "01:28", "", COLOR_GREEN);
        drawDronePanel(comp, "HUD_map_area", 82, 1100, 420, 190, "AREA DONE", "62", "%", COLOR_GREEN);

        drawDronePanel(comp, "HUD_map_alt", 3338, 410, 420, 190, "ALTITUDE", "120.4", "m", COLOR_ACCENT);
        drawDronePanel(comp, "HUD_map_spd", 3338, 640, 420, 190, "SPEED", "10.2", "m/s", COLOR_WHITE);
        drawDronePanel(comp, "HUD_map_home", 3338, 870, 420, 190, "HOME", "860", "m", COLOR_WHITE);
        drawDronePanel(comp, "HUD_map_heading", 3338, 1100, 420, 190, "HEADING", "045", "deg", COLOR_GREEN);
    }

    function drawMapCoordinates(comp) {
        var panel = makeShapeLayer(comp, "HUD_map_coordinates");
        var c = addGroup(panel, "coordinate panel");
        addRectToContents(c, 650, 1765, 2540, 150, 10);
        addFill(c, COLOR_BLACK, 38);
        addStroke(c, COLOR_WHITE, 3, 65);
        addText(comp, "HUD_map_lat", "LAT  13.7563 N", 725, 1848, 32, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_map_lon", "LON  100.5018 E", 1110, 1848, 32, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_map_home_coord", "HOME  860 m", 1545, 1848, 32, COLOR_GREEN, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_map_dist_remaining", "DIST REMAINING  2.45 km", 1950, 1848, 32, COLOR_ACCENT, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_map_time_remaining", "TIME  18:47", 2585, 1848, 32, COLOR_YELLOW, ParagraphJustification.LEFT_JUSTIFY, 8);
    }

    function drawMapLegend(comp) {
        var panel = makeShapeLayer(comp, "HUD_map_legend");
        var c = addGroup(panel, "legend panel");
        addRectToContents(c, 3338, 1390, 420, 330, 10);
        addFill(c, COLOR_BLACK, 42);
        addStroke(c, COLOR_WHITE, 3, 70);
        addText(comp, "HUD_map_legend_title", "LEGEND", 3380, 1445, 28, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 6);

        var marks = addGroup(panel, "legend marks");
        addLineToContents(marks, 3385, 1508, 3455, 1508);
        addLineToContents(marks, 3385, 1578, 3455, 1578);
        addEllipseToContents(marks, 3415, 1634, 28, 28);
        addStroke(marks, COLOR_GREEN, 4, 100);
        var current = addGroup(panel, "current mark");
        addLineToContents(current, 3385, 1578, 3455, 1578);
        addStroke(current, COLOR_YELLOW, 5, 100);
        var point = addGroup(panel, "point fill");
        addEllipseToContents(point, 3415, 1634, 28, 28);
        addFill(point, COLOR_WHITE, 100);
        addText(comp, "HUD_map_legend_route", "ROUTE", 3480, 1518, 25, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 4);
        addText(comp, "HUD_map_legend_current", "CURRENT", 3480, 1588, 25, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 4);
        addText(comp, "HUD_map_legend_wp", "WAYPOINT", 3480, 1658, 25, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 4);
    }

    function drawDroneMapInterfaceHUD(comp) {
        drawMapTopStatus(comp);
        drawDroneFrame(comp);
        drawMapPanel(comp);
        drawMapSideStats(comp);
        drawMapCoordinates(comp);
        drawMapLegend(comp);
        drawDroneBottomStatus(comp);
    }

    if (!app.project) app.newProject();
    app.beginUndoGroup("Drone Map Interface HUD Builder RV v1");
    try {
        ...
        alert("Drone Map Interface HUD Builder problems:\n\n- " + err.toString() + (err.line ? " line " + err.line : ""));
    } finally {
        app.endUndoGroup();
    }
})();
