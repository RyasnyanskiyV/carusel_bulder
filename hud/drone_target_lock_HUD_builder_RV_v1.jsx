#target aftereffects
#targetengine "DroneTargetLockHUDBuilderRVV1"

/*
    drone_target_lock_HUD_builder_RV v1
    Adobe After Effects 2022+

    Procedural drone target lock HUD overlay.
...
(function DroneTargetLockHUDBuilderRVV1() {
    var COMP_W = 3840;
    var COMP_H = 2160;
    var COMP_DURATION = 10;
    var FPS = 30;
    var COMP_BASE = "DRONE_TARGET_LOCK_HUD_MAIN";
    ...
    function drawTargetLockBox(comp) {
        var layer = makeShapeLayer(comp, "HUD_target_lock_box");
        var c = addGroup(layer, "target brackets");
        var x = 1515;
        var y = 660;
        var w = 810;
        var h = 620;
        ...
        addStroke(c, COLOR_GREEN, 6, 100);

        var inner = addGroup(layer, "inner scan marks");
        addLineToContents(inner, x + 260, y + h * 0.5, x + 350, y + h * 0.5);
        addLineToContents(inner, x + 460, y + h * 0.5, x + 550, y + h * 0.5);
        addLineToContents(inner, x + w * 0.5, y + 215, x + w * 0.5, y + 285);
        addLineToContents(inner, x + w * 0.5, y + 335, x + w * 0.5, y + 405);
        addEllipseToContents(inner, x + w * 0.5 - 52, y + h * 0.5 - 52, 104, 104);
        addStroke(inner, COLOR_WHITE, 3, 82);

        var vector = addGroup(layer, "motion vector");
        addLineToContents(vector, x + w * 0.5, y + h * 0.5, x + w * 0.5 + 275, y + h * 0.5 - 145);
        addLineToContents(vector, x + w * 0.5 + 275, y + h * 0.5 - 145, x + w * 0.5 + 232, y + h * 0.5 - 145);
        addLineToContents(vector, x + w * 0.5 + 275, y + h * 0.5 - 145, x + w * 0.5 + 255, y + h * 0.5 - 105);
        addStroke(vector, COLOR_YELLOW, 4, 90);

        addText(comp, "HUD_target_status", "TARGET LOCK", 1920, 595, 46, COLOR_GREEN, ParagraphJustification.CENTER_JUSTIFY, 12);
        addText(comp, "HUD_target_id", "ID: OBJ-042", x, y + h + 68, 30, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_target_tracking", "TRACKING ACTIVE", x + w, y + h + 68, 30, COLOR_GREEN, ParagraphJustification.RIGHT_JUSTIFY, 8);
    }

    function drawTargetRangeRings(comp) {
        var cx = 1920;
        var cy = 970;
        var layer = makeShapeLayer(comp, "HUD_target_range_rings");
        var c = addGroup(layer, "rings");
        addEllipseToContents(c, cx - 300, cy - 300, 600, 600);
        addEllipseToContents(c, cx - 450, cy - 450, 900, 900);
        for (var i = 0; i < 24; i++) {
            var a = (i * 15) * Math.PI / 180;
            var r1 = i % 3 === 0 ? 440 : 462;
            var r2 = 485;
            addLineToContents(c, cx + Math.cos(a) * r1, cy + Math.sin(a) * r1, cx + Math.cos(a) * r2, cy + Math.sin(a) * r2);
        }
        addStroke(c, COLOR_DIM, 2, 42);
    }

    function drawTargetConfidence(comp) {
        var panel = makeShapeLayer(comp, "HUD_target_confidence_panel");
        var c = addGroup(panel, "panel");
        addRectToContents(c, 1260, 1370, 1320, 112, 10);
        addFill(c, COLOR_BLACK, 42);
        addStroke(c, COLOR_WHITE, 3, 65);

        addText(comp, "HUD_target_conf_label", "LOCK CONFIDENCE", 1310, 1438, 28, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_target_conf_value", "94%", 2480, 1438, 42, COLOR_GREEN, ParagraphJustification.RIGHT_JUSTIFY, 6);

        var bar = makeShapeLayer(comp, "HUD_target_confidence_bar");
        var bg = addGroup(bar, "bar background");
        addRectToContents(bg, 1600, 1415, 660, 22, 3);
        addFill(bg, COLOR_DIM, 45);
        var fill = addGroup(bar, "bar fill");
        addRectToContents(fill, 1600, 1415, 620, 22, 3);
        addFill(fill, COLOR_GREEN, 100);
    }

    function drawTargetDataPanels(comp) {
        drawDronePanel(comp, "HUD_target_range", 82, 410, 420, 190, "TARGET RANGE", "218", "m", COLOR_YELLOW);
        drawDronePanel(comp, "HUD_target_speed", 82, 640, 420, 190, "TARGET SPEED", "8.4", "m/s", COLOR_WHITE);
        drawDronePanel(comp, "HUD_target_bearing", 82, 870, 420, 190, "BEARING", "037", "deg", COLOR_GREEN);
        drawDronePanel(comp, "HUD_target_delta_alt", 82, 1100, 420, 190, "DELTA ALT", "-14", "m", COLOR_ACCENT);

        drawDronePanel(comp, "HUD_target_drone_alt", 3338, 410, 420, 190, "DRONE ALT", "120.4", "m", COLOR_ACCENT);
        drawDronePanel(comp, "HUD_target_drone_speed", 3338, 640, 420, 190, "DRONE SPD", "12.6", "m/s", COLOR_WHITE);
        drawDronePanel(comp, "HUD_target_gps", 3338, 870, 420, 190, "GPS", "18", "sat", COLOR_GREEN);
        drawDronePanel(comp, "HUD_target_signal", 3338, 1100, 420, 190, "LINK", "92", "%", COLOR_GREEN);
    }

    function drawTargetBottomStrip(comp) {
        var strip = makeShapeLayer(comp, "HUD_target_bottom_strip");
        var c = addGroup(strip, "strip");
        addRectToContents(c, 610, 1980, 2620, 92, 10);
        addFill(c, COLOR_BLACK, 38);
        addStroke(c, COLOR_WHITE, 3, 60);
        addText(comp, "HUD_target_bottom_mode", "MODE TARGET LOCK", 665, 2040, 30, COLOR_GREEN, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_target_bottom_gps", "GPS 3D", 1125, 2040, 30, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_target_bottom_range", "RANGE 218 m", 1450, 2040, 30, COLOR_YELLOW, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_target_bottom_eta", "ETA 00:00:18", 1905, 2040, 30, COLOR_WHITE, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_target_bottom_rc", "RC STRONG", 2385, 2040, 30, COLOR_DIM, ParagraphJustification.LEFT_JUSTIFY, 8);
        addText(comp, "HUD_target_bottom_bat", "BAT 78%", 2780, 2040, 30, COLOR_GREEN, ParagraphJustification.LEFT_JUSTIFY, 8);
    }

    function drawTargetMiniRadar(comp) {
        var panel = makeShapeLayer(comp, "HUD_target_mini_radar");
        var c = addGroup(panel, "radar panel");
        addRectToContents(c, 2920, 1455, 640, 360, 10);
        addFill(c, COLOR_BLACK, 42);
        addStroke(c, COLOR_WHITE, 3, 75);

        var radar = addGroup(panel, "radar marks");
        addEllipseToContents(radar, 3090, 1535, 300, 300);
        addEllipseToContents(radar, 3150, 1595, 180, 180);
        addLineToContents(radar, 3240, 1520, 3240, 1840);
        addLineToContents(radar, 3080, 1680, 3400, 1680);
        addStroke(radar, COLOR_DIM, 2, 65);

        var target = addGroup(panel, "target blip");
        addEllipseToContents(target, 3330, 1590, 34, 34);
        addFill(target, COLOR_RED, 100);
        addText(comp, "HUD_target_radar_label", "TARGET", 3240, 1508, 28, COLOR_DIM, ParagraphJustification.CENTER_JUSTIFY, 6);
    }

    function drawDroneTargetLockHUD(comp) {
        drawDroneTopStatus(comp);
        drawDroneFrame(comp);
        drawTargetRangeRings(comp);
        drawTargetLockBox(comp);
        drawTargetConfidence(comp);
        drawTargetDataPanels(comp);
        drawDroneCompass(comp);
        drawTargetMiniRadar(comp);
        drawTargetBottomStrip(comp);
    }

    if (!app.project) app.newProject();
    app.beginUndoGroup("Drone Target Lock HUD Builder RV v1");
    try {
        ...
        alert("Drone Target Lock HUD Builder problems:\n\n- " + err.toString() + (err.line ? " line " + err.line : ""));
    } finally {
        app.endUndoGroup();
    }
})();
