...
/*
    ...
    - creates exactly the requested number of text blocks
...
(function CaruselBuilderRVTextV83(thisObj) {
    var VERSION = "Text 8.3";
    var FILM_SIZE_MM = 36;
    var BASE_FOCAL_LENGTH_MM = 35;
    var MAX_ALLOWED_TEXT_BLOCKS = 40;
    ...
    var FX_TEXT_RADIUS = "Text Radius";
    ...
    function makePanel(thisObj) {
        ...
        var note = setup.add(
            "statictext",
            undefined,
            "Creates exactly the number of text blocks requested here.",
            { multiline: true }
        );
    ...
    function addTextController(comp, cardWidth, cardHeight, initialRadius) {
        var ctrl = comp.layers.addNull();
        ctrl.name = CARD_CTRL;
        ctrl.label = 11;
        ctrl.property("ADBE Transform Group").property("ADBE Position")
        ...
        addSlider(
            fx,
            FX_TEXT_RADIUS,
            Math.max(0, Math.round(initialRadius || 0))
        );
    ...
    function makePositionExpression(count) {
        ...
        return 'c=thisComp.layer("' + MOVE_CTRL + '");\n' +
            ...
            slideCardMetrics +
            'r=Math.max(0,cardCtrl.effect("' + FX_TEXT_RADIUS +
            '")("Slider"));\n' +
    ...
    function makeScaleExpression(count) {
        return 'c=thisComp.layer("' + MOVE_CTRL + '");\n' +
            ...
            '[100*k*appear,100*k*appear,100*k*appear];';
    }
    ...
    function addRigAndSlides(
        ...
        var positionExpression = makePositionExpression(count);
        var scaleExpression = makeScaleExpression(count);
        var opacityExpression = makeOpacityExpression(count);
    ...
    function resetTextController(textCtrl, slides, issues) {
    ...
    function updateActiveRig() {
        ...
        for (var i = 0; i < slides.length; i++) {
            ...
            assignExpression(
                ...
                makePositionExpression(slides.length),
                slide.name + " position",
                issues
            );
            assignExpression(
                slide.property("ADBE Transform Group")
                    .property("ADBE Scale"),
                makeScaleExpression(slides.length),
                slide.name + " scale",
                issues
            );
            assignExpression(
                ...
                makeOpacityExpression(slides.length),
    ...
    function resetActiveRig() {
        ...
        resetTextController(textCtrl, slides, issues);
    ...
    function createFromPanel(ui) {
        ...
        count = clamp(count, 1, MAX_ALLOWED_TEXT_BLOCKS);
        ...
        try {
            ...
            ui.status.text = result.issues && result.issues.length > 0
                ? "Problems found"
                : "";
